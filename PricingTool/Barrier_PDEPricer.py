# -*- coding: utf-8 -*-
"""
Created on Sat Apr 20 17:17:44 2019
...
class Barrier_PDEPricer():
    ...
    def optionVal(self, payoff, stockGridNum = 100, timeGridNum = 500, method = 'Explicit', logSpace = False, American = False):
        ...
        if logSpace:
            ...
            if method == 'Crank-Nicolson':
                ...
                b = (Barrier_PDEPricer.r/2 - 1/self.dt + 0.25*2*self.vol**2/self.dx**2)*np.ones(stockGridNum+1)
                c = (0.25*mu/self.dx - 0.25*self.vol**2/self.dx**2)*np.ones(stockGridNum+1)
                d = (0.25*-2*self.vol**2/self.dx**2 - 1/self.dt-Barrier_PDEPricer.r/2)*np.ones(stockGridNum+1)
            ...
            if method == 'Explicit':
                ...
                for j in range(timeGridNum):
                    ...
                    V[0]=payoff(self.S[0])
                    V[stockGridNum] = payoff(s_max*np.exp(self.T*(Barrier_PDEPricer.r-self.d)))*np.exp(Barrier_PDEPricer.r*self.dt*(timeGridNum-j-1))
                    Vmatrix[:,-2-j] = V
                self.Vmatrix = Vmatrix
                return V[int(stockGridNum/2)]
    ...
    def delta(self):
        Vm = self.Vmatrix
        if self.logSpace:
            return (Vm[int(self.stockGridNum/2)+1,0] - Vm[int(self.stockGridNum/2)-1,0])/(self.S0 * (np.exp(self.dx)-np.exp(-self.dx)))
        else:
            ds = self.ds
